/**
  ...
  */
package com.yuwnloy.gxgtools.compression;
...
import java.util.Arrays;
 ...
 public class LZSS {
     ...
     public static byte[] compress(InputStream input) throws IOException {
         ...
         byte[] ringBuffer = new byte[RING_SIZE + MAX_STORE_LENGTH - 1];
         short[] dad = new short[RING_SIZE + 1];
         short[] leftSon = new short[RING_SIZE + 1];
         short[] rightSon = new short[RING_SIZE + 257];
         
         // Start with a clean tree.
         initTree(dad, leftSon, rightSon);
         ...
         int filledCount = r>16?16:r;
         int needs = r - filledCount;
         for(int j=0;j<filledCount;j++){
        	 ringBuffer[j] = 32;
         }
         while(needs > 0){
        	 int filled = filledCount>needs?needs:filledCount;
        	 System.arraycopy(ringBuffer, 0, ringBuffer, filledCount, filled);
        	 filledCount += filled;
        	 needs = r-filledCount;
         }
//         System.err.println("filled->"+filledCount+"     r->"+r);
         ...
         for (i = 1; i <= MAX_STORE_LENGTH; i++) {
             int[] results = insertNode((short) (r - i), matchLength, matchPosition, ringBuffer, dad, leftSon, rightSon);
             matchLength = (short)results[0];
             matchPosition = results[1];
         }
 
         // Finally, insert the whole string just read. The
         // member variables matchLength and matchPosition are set.
         int[] results = insertNode(r, matchLength, matchPosition, ringBuffer, dad, leftSon, rightSon);
         matchLength = (short)results[0];
         matchPosition = (short)results[1];
         ...
         do {
             ...
             for (i = 0; i < lastMatchLength; i++) {
                 ...
                 // Delete "old strings"
                 deleteNode(s, dad, leftSon, rightSon);
                 ...
                 results = insertNode(r, matchLength, matchPosition, ringBuffer, dad, leftSon, rightSon);
                 matchLength = (short)results[0];
                 matchPosition = (short)results[1];
             }
     ...
     public static byte[] uncompressAsByteArray(byte[] input) throws IOException {
    	 ByteArrayInputStream is = new ByteArrayInputStream(input);
    	 return uncompressAsByteArray(is, Integer.MAX_VALUE, BUF_SIZE);
     }
      ...
      */
     public static InputStream uncompressAsInputStream(InputStream input) throws IOException {
    	 return uncompressAsInputStream(input, Integer.MAX_VALUE);
     }
      ...
      * @see org.crosswire.common.compress.Compressor#uncompress()
      */
     public static InputStream uncompressAsInputStream(InputStream input, int length) throws IOException {
    	 return uncompressAsInputStream(input, length, BUF_SIZE);
     }
      ...
      */
     public static byte[] uncompressAsByteArray(InputStream input, int length, int buffSize) throws IOException {
         ...
         while (true) {
             ...
             if ((flags & 1) != 0) {
            	 ...
            	 int canRead = length - readCount;
            	 if(canRead < 2){
            		 input.read(c, 0, canRead);
            		 break;
            	 }
     ...
     public static InputStream uncompressAsInputStream(InputStream input, int length, int buffSize) throws IOException {
         return new ByteArrayInputStream(uncompressAsByteArray(input, length, buffSize));
     }
 
     /**
      * Initializes the tree nodes to "empty" states.
      */
     private static void initTree(short[] dad, short[] leftSon, short[] rightSon) {
      ...
      */
     private static int[] insertNode(short pos, int matchLength, int matchPosition, byte[] ringBuffer, short[] dad, short[] leftSon, short[] rightSon) {
         ...
         while (true) {
             if (cmp >= 0) {
                 if (rightSon[p] != NOT_USED) {
                     ...
                     rightSon[p] = pos;
                     dad[pos] = p;
                     return new int[]{matchLength, matchPosition};
                 }
             } else {
      ...
      *            the node to remove
      */
     private static void deleteNode(short node, short[] dad, short[] leftSon, short[] rightSon) {
